...
\end{array}\right)
$$
The `saveDirectory` is required to be provided. The package will create the directory `/SaveDirectory/analysisId` where the analysis specific results will be stored.

We can create the analysis settings using the following code:

```{r tidy=FALSE, eval=FALSE}
analysisSettings <- createAnalysisSettings (analysisId = "vignette_demonstration",
...
```{r tidy=FALSE, eval=FALSE}
databaseSettings <- createDatabaseSettings(cdmDatabaseSchema = "cdm_truven_mdcd_v780.dbo",
