...
This vignette describes tha application of a risk modeling predictive approach to the assessment of treatment effect heterogeneity (HTE) using the `RiskStratifiedEstimation` package. The method involves 5 main steps summarized in Figure \ref{workflow}:
...
- **Evaluation of the results:** The package provides standardized output to assess the results. First, it allows assessment of  the quality of the results, e.g. follow-up times, outcome rates, performance of prediction models, overalp of the propensity score distributions and others. Second, it allows for quick and standardized assessment of absolute and relative treatment effects in risk strata.


![RiskStratifiedEstimation package workflow \label{workflow}](rsee_workflow.JPG)

# Study specification and database selection
For the current demonstration we consider some of the treatments and outcomes assesed in the Large-scale Evidence Generation in Network of Databaseses (LEGEND) study for hypertension. The treatment cohort contains patients with a hypertension diagnosis within the database that receive angiotensin-converting enzyme (ACE) inhibitors, followed from the time of initiation until the time of cessation. The comaparator cohort contains the set of patients within the database with a hypertension diagnsis that receive beta blockers. We also cosider 2 outcome cohorts. The first contains all patients found in the database that have at least one occurence of cardiovascular disease (CVD). The second contains all patients with an ocurence of cough. We consider hospitalization with heart failure (HF-hosp) as the main outcome of interest that we are trying to prevent in the study and cough as a common side-effect of ACE-inhibitors. Cohort defintions are available online [here](https://github.com/OHDSI/Legend).

After defining the problem we need to define the database on which we will assess treatment effect heterogeneity. The aim is the inclusion of multiple databases that the patient population.


# Study implementation

## Cohort instantiation
Before we can use the package we need to set up the cohorts of our analyses. For that we can use `ATLAS` or write our own script for the extraction of the data. In any case, we assume that the data have been extracted in some way from a database mapped to OMOP-CDM and have been stored to tables where we have read and write permissions.

## Study script creation
For this part 7 types of input need to be created:
...
### Analysis settnigs
The analysis settings refer to information that define the structure of the study and can be used to identify it. The `analysisId` should be a unique identifier of the present study.

The `treatmentCohortId` refers to the unique identifier of the treatment cohort definition. In our case, that is $1001$ and refers to the extracted hypertensive patients receving ACE-inhibitors. The `comparatorCohortId` refers to the unique identifier of the comparator cohort. In our case this is $1002$ and refers to the extracted cohort of hypertensive patients receiving beta blockers; the outcome ids that refer to the unique identifiers of the outcome cohorts. 

The `outcomeIds` refer to the outcomes (both main and safety) we are interested in analyzing. In our case the $153$ refers to HF-hosp and $134$ to cough events.

A very important characteristic of the analysis settings is the `analysisMatrix`, that dictates which comparisons are to be made. The columns of this square boolean matrix refer to risk stratification outcomes and the rows to estimation outcomes. Note that the order of the `mapMatrix` columns matches the order of the outcomes defined in the `outcomeIds` part of the `analysisSettings`. The default is the diagonal matrix, so in that case we would have:
$$
\text{analysisMatrix} = \left(\begin{array}{cc} 
1 & 0\\
0 & 1
\end{array}\right)
$$
The default `mapMatrix` defines the following analyses:

- Within strata of hospitalization with heart failure risk estimate relative and absolute treatment effects of HF-hosp events.
- Within strata of cough risk estimate relative and absolute treatment effects of coughing events.

In our case we will make an extra analysis, where within strata of predicted HF-hosp we estimate relative and absolute treatment effects of cough. Therefore, our `analysisMatrix` will be of the form
$$
\text{analysisMatrix} = \left(\begin{array}{cc} 
1 & 0\\
1 & 1
\end{array}\right)
$$
The `saveDirectory` is required. The package will create the directory `/SaveDirectory/analysisId` where the analysis-specific results will be stored.
...
analysisSettings <- 
  createAnalysisSettings(
    ...
    outcomeIds = c(153, 134),
    analysisMatrix = matrix(c(1, 1, 0, 1), ncol = 2),
    mapTreatments = data.frame(
      idNumber = c(1001, 1002),
      label = c("ACE inhibitors", "Beta blockers")),
    mmapOutcomes = data.frame(
      idNumber = c(153, 134),
      label = c("Heart failure (hosp)", "Cough")),
    verbosity = "INFO",
...
```{r tidy=FALSE, eval=FALSE}
getDataSettings <- createGetDataSettings(getPlpDataSettings = 
                                           createGetPlpDataArgs(washoutPeriod = 365),
                                         getCmDataSettings = 
                                           createGetCmDataArgs(washoutPeriod = 365))
```
...
```{r tidy=FALSE, eval=FALSE}
covariateSettings <- 
  createGetCovariateSettings(covariateSettingsCm = 
...
```{r tidy=FALSE, eval=FALSE}
populationSettings <- 
  createPopulationSettings(populationPlpSettings = 
                             createPopulationPlpSettingsArgs(
                               riskWindowEnd = 365,
                               minTimeAtRisk = 364),
                           populationCmSettings =
                             createPopulationCmSettingsArgs(
...
```{r tidy=FALSE, eval=FALSE}
runSettings <- 
  createRunSettings(runPlpSettings = 
                      createRunPlpSettingsArgs(),
                    runCmSettings = 
